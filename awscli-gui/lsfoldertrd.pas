...
begin
  try
    ...
    if MainForm.GroupBox2.Caption = 's3://' then
      ExProcess.Parameters.Add('aws ' + endpoint_url +
        ' s3 ls | cut -d " " -f3 | awk ' + '''' + '{ print $0 "/"}' + '''' + ' | sort')
    else
      ExProcess.Parameters.Add('aws ' + endpoint_url + ' s3 ls ' +
        MainForm.GroupBox2.Caption + ' | cut -b32-');
